...
class JoinMeetingViewModel extends ChangeNotifier {
  ...
  Future<bool> joinMeeting(MeetingViewModel meetingProvider, MethodChannelCoordinator methodChannelProvider) async {
    ...
    final ApiResponse? apiResponse = await api.join(meetingProvider.phoneNumber!);
    ...
    if (apiResponse.response && apiResponse.content != null) {
      ...
      log(meetingProvider.phoneNumber ?? "");
      log(meetingProvider.dtmsEnabled ?? "");
      log(meetingProvider.dtmsSettings ?? "");
    }
    ...
    if (joinResponse.result) {
      ...
      // await player.resume();
      // await player.play(AssetSource("call-ringtone.mp3"));
      Timer(const Duration(seconds: 20,),(){
        if(!meetingProvider.callAnswered) {
